...
Run 2 beats both the old rule and run 1 on fill rate, stockout cost, and profit. Fill rate no longer falls at all — it ticks up slightly. Stockout cost falls slightly instead of rising. Bullwhip still falls sharply (see table above), and now that improvement comes without the fill-rate and cost damage seen in run 1. Holding cost is the one number where run 1 shows a bigger drop (−$493M vs. run 2's −$35.1M) — but run 1's drop came from nodes running out of stock, not from efficient inventory management: it paired with the −12.2 pt fill-rate collapse and the $2.0B stockout-cost spike above. Emptier shelves mean less inventory to hold; that is not a saving worth having. Run 2's smaller, healthier holding-cost drop comes alongside better fill rate and lower stockout cost, which is the outcome that matters.
...
**Shanghai chain (5 nodes).** The smallest group: one DC in Shanghai and its four stores. A control tower smooths their ordering a little (bullwhip drops by about 1 unit at the DC, 3 at the manufacturer, 14 at the contract manufacturer). Customers are served about as well as before — fill rate is flat, and stockout cost is flat to slightly lower. Profit is about flat too. Holding cost rises a little here too (+$5.4 million), the same pattern seen in the downstream arm but smaller. Unlike run 1, this arm no longer hurts customers.

**Downstream (12 nodes: all 4 DCs and all 8 stores).** With every DC and store using a control tower, the smoothing effect is bigger (bullwhip drops more at every tier: about 2 units at DCs, 8 at manufacturers, 39 at contract manufacturers). Fill rate improves slightly (about 0.1 percentage points better), stockout cost falls about $15.7 million, and profit is about $26.5 million higher, though that last number is not statistically distinguishable from zero. Holding cost and scrap cost both rise in this arm (holding +$17.4 million, scrap +$8.3 million) — the towers are carrying more echelon safety stock and building more product than they end up needing, even though customers are better served.
...
## How the safety stock is set

A measurement on seed 1, whole-network adoption, 156 weeks (throwaway script, not part of the repo) checked how often that floor is the binding term — i.e., how often it is at least as large as the pooled echelon number (`z × sigma_end × sqrt(horizon)`) it's compared against. The floor won essentially every time: 100% of the calls at the DC tier, 100% at MFG, and 100% at CM. In other words, the pooled echelon safety-stock number is almost never actually used — the towers are, in practice, just carrying the sum of the safety stock the downstream nodes would have kept anyway. That means the fill-rate and cost gains reported above come from the echelon rule's smoother, whole-chain-aware ordering (seeing real stock and demand across the network) rather than from genuinely pooling safety stock into one smaller number — the classic textbook promise of echelon inventory control (holding less total safety stock by sharing it across a chain) is not what is happening here.
