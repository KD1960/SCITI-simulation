...
- **2026-09-23, runs done and scored:** `prereg-llm-run-results.md`. Prices on the day $2 / $10 per MTok; `sciti estimate` $7.26 per run; approved cap $12 per run; actual $26.65 for ten runs. All ten replay exactly. Verdicts: G1 right (line), G2 **wrong** (5/5), G3 right, G4 line not met (3/5) but LLM ahead on average, G5 **wrong**, G6 judgment, G7 right.
