...
**Last updated:** 2026-09-18 (catalog v2 with evidence-based values on `main`; 210 tests pass)
**Phase:** MVP merged to `main`; engine corrected after the 2026-09-17 test audit. See README.md for how to use it.
**Next step:** rerun E2–E5 on catalog v2 using `network_profit_with_inventory` (E5 is a paid LLM run: ask Kevin first; suppliers now use rules). Then check the shrink baseline (0.2%/week) and collect cost evidence for control tower, APS, and ML forecasting. Kevin's wish list, in his suggested order: disruption library, live step-by-step view, role-playing game.
