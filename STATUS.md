...
**Last updated:** 2026-09-18 (catalog v2 on `main`; E1–E4 rerun on it; 210 tests pass)
**Phase:** MVP merged to `main`; engine corrected after the 2026-09-17 test audit. See README.md for how to use it.
**Next step:** E5 rerun (paid LLM run on the corrected engine and catalog v2, suppliers on rules, about $2 a run: ask Kevin first, set prices in `configs/mvp_llm.yaml`). Then: check the shrink baseline (0.2%/week); cost evidence for control tower, APS, and ML forecasting; consider a random-disruption experiment using the evidence table's base rates.
