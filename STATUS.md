...
**Next step:** Kevin rules on `docs/sciti2/evidence-table.md` §4 (catalog v2 values and two model-shape changes); then build catalog v2, rerun E1 and the sensitivity screen on it, and rerun E2–E5 using `network_profit_with_inventory`. Kevin's wish list, in his suggested order: disruption library, live step-by-step view, role-playing game.
