...
**Next step (Kevin to choose):** (a) more LLM seeds, calm and disrupted, about $3 each, to turn the seed-1 patterns into estimates (include an unforecastable seed); (b) close the blockchain gap: show each technology's expected saving for this firm in the brief, and/or loosen the one-per-quarter quota; (c) a hybrid policy (payback rule + an insurance heuristic) as a free benchmark; (d) a random-disruption experiment using the evidence table's base rates. Also open: shrink baseline (0.2%/week); cost evidence for control tower, APS, and ML forecasting.
