...
**Last updated:** 2026-09-19 (catalog v2 on `main`; E1–E5 rerun on it; suppliers-follow rule, insurance habit, and random-disruption experiment added; 213 tests pass)
**Phase:** MVP merged to `main`; engine corrected after the 2026-09-17 test audit. See README.md for how to use it.
**Next step:** Kevin rules on modelling implementation failure (`docs/sciti2/implementation-failure-evidence.md` §5). Other open options: (a) LLM event feed test (paid); (b) LLM blockchain gap; (c) LLM agents on random disruptions (paid); (d) model refinements (risk intel conditional on buffers, first-mover scarcity, varied severity, regional events). Also open: shrink baseline (0.2%/week); cost evidence for control tower, APS, and ML forecasting.
