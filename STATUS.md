...
**Next step (Kevin to choose):** (a) give LLM agents an event feed in the brief and test whether they react to disruptions (paid, ~$3/run); (b) close the LLM blockchain gap (expected saving per firm in the brief; quota); (c) LLM agents on the random-disruption scenario at rate 0.2, several seeds (paid); (d) model refinements the evidence points to: risk intel's benefit conditional on buffers or a second source, first-mover scarcity, varied severity and regional events. Also open: shrink baseline (0.2%/week); cost evidence for control tower, APS, and ML forecasting.
