...
**Last updated:** 2026-09-18 (catalog v2 on `main`; E1–E5 rerun on it; suppliers-follow rule and insurance habit added; 213 tests pass)
**Phase:** MVP merged to `main`; engine corrected after the 2026-09-17 test audit. See README.md for how to use it.
**Next step (Kevin to choose):** (a) a random-disruption experiment using the evidence table's base rates, to give the insurance habit and risk intel an expected value; (b) give LLM agents an event feed in the brief and see whether they react to disruptions (paid); (c) close the LLM blockchain gap (expected saving per firm in the brief; quota); (d) more LLM seeds. Also open: shrink baseline (0.2%/week); cost evidence for control tower, APS, and ML forecasting; risk intel's 2-weeks-saved value is low-confidence and now carries a lot of weight.
