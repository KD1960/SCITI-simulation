...
**Next step (Kevin to choose):** (a) rule on risk intel's form: saved = min(2, 0.25 × outage) and warning 30% (small engine change + rerun of the random-disruption experiment); (b) give LLM agents an event feed in the brief and test whether they react to disruptions (paid, ~$3/run); (c) close the LLM blockchain gap (expected saving per firm in the brief; quota); (d) LLM agents on the random-disruption scenario at rate 0.2, several seeds (paid). Also open: shrink baseline (0.2%/week); cost evidence for control tower, APS, and ML forecasting; varied disruption severity and regional events.
